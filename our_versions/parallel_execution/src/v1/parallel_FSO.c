...
void initFish(Fish *fish, int rank, int size) {
    ...
    double portion_bounds = (SPAWN_BOUNDS_MAX - SPAWN_BOUNDS_MIN) / size; // Calcola la porzione corretta per ciascun banco
...
void individualMovementArray (Fish *fishArray, int n_fishes, int current_iteration, float *local_tot_delta_fitness, float *global_tot_delta_fitness, float *local_weighted_tot_delta_fitness, float *global_weighted_tot_delta_fitness, float *local_max_delta_fitness_improvement, float *global_max_delta_fitness_improvement) {
    ...
    if (current_iteration % UPDATE_FREQUENCY == 0) {
        ...
        *global_tot_delta_fitness = *local_tot_delta_fitness;
        #pragma omp parallel for
        for (int d = 0; d < DIMENSIONS; d++)
        {
            global_weighted_tot_delta_fitness[d] = local_weighted_tot_delta_fitness[d];
        }
...
void breeding(Fish *fishes, int n_fishes, int current_iteration, int rank, int num_ranks) {
    int first_index = 0;
    int second_index = 0;
    int worst_index = 0;
