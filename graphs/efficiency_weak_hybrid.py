...
with open("../implementations/hybrid/results_weak_modified.json", "r") as f:
    data = json.load(f)
...
for entry in selected_data:
    cores_group[entry["cores"]].append((entry["nodes"], entry["efficiency_weak"]))

# Plot
plt.figure(figsize=(10, 6))
for core, values in cores_group.items():
    values.sort()
    nodes, efficiencies = zip(*values)
    if core == 1:
        plt.plot(nodes, efficiencies, marker='o', label=f"{core} thread")
    else:
        plt.plot(nodes, efficiencies, marker='o', label=f"{core} threads")
        

plt.xlabel("Processes")
plt.ylabel("Efficiency")
plt.title(f"Efficiency vs Processes (density={selected_key[0]:.2f}, place={selected_key[1]})")
plt.legend(title="Number of Threads")
plt.grid(True)
plt.ylim(0, 1.3)  # Efficiency scale
...
plt.tight_layout()
plt.savefig(f"images/efficiency_vs_nodes_hybrid_weak_{selected_key[0]:.2f}_{selected_key[1]}.png", dpi=300)
# plt.show()
